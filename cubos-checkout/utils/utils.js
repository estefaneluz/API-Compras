async function verificarEstoque(produtos, id, quantidade) {
  return produtos.find(
    (produto) => produto.id === parseInt(id) && produto.estoque >= quantidade
  );
}
...
async function atualizarCarrinho(data, produto, quantidade) {
  ...
  carrinho.valorDoFrete =
    carrinho.subtotal <= 20000 && carrinho.produtos.length != 0 ? 5000 : 0;
...
async function validarUsuario(userInfo) {
  ...
  if (!type || !country || !name || !documents) {
    erros.push(
      "Está faltando dados do cliente. Precisa conter: type, country, name e documents."
    );
    return erros;
  }
  ...
  const validarDocuments = documents.some((documento) => {
    return (
      documento.hasOwnProperty("type") &&
      documento.hasOwnProperty("number") &&
      documento.type.toLowerCase() === "cpf" &&
      documento.number.length === 11 &&
      validarCpf(documento.number)
    );
  });
